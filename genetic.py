...
tones = ['C4','#C4','D4','#D4','E4','F4','#F4','G4','#G4','A4','#A4','B4']
...
def generate_tone():
    ...
    tone = tones[idx]
    print(f"Choose tone: {tone}, idx: {idx}")
    return 8 + idx
...
def normalize_weights(scores, std_before_exp=1.):
    ...
    print(scores)
    mean, std = np.mean(sorted_scores[1:-1]), np.std(sorted_scores[1:-1])
...
def genetic_algorithm(pop_size=10, 
    ...
    for generation in range(generations):
        # 计算适应度
        fitness_scores = [fitness(melody, tone) for melody in population]
